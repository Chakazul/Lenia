...
class Board:
	...
	def transform(self, rotate, new_R, shift=[0,0], is_replace=False, reshape=True):
		A = np.roll(self.cells, shift, (0, 1))
		A = snd.rotate(A, rotate, order=0, reshape=reshape, mode='constant' if reshape else 'wrap')
...
class Lenia:
	...
	def key_press(self, event):
		...
		# elif k in ['f']: self.zoom = max(1, min(20, self.zoom - 1)); self.load_animal_ID(self.animal_ID)
		elif k in ['r', 'S+r']: self.zoom = max(1, min(20, self.zoom *  2)); self.world.params['R'] = max(1, min(max(SIZEX,SIZEY), self.world.params['R'] *  double_mul + double_add)); self.automaton.calc_kernel(); self.transform_world()
		elif k in ['f', 'S+f']: self.zoom = max(1, min(20, self.zoom // 2)); self.world.params['R'] = max(1, min(max(SIZEX,SIZEY), self.world.params['R'] // double_mul - double_add)); self.automaton.calc_kernel(); self.transform_world()
		elif k in ['down',  'S+down']:  self.shift[0] = self.shift[0] - linear_add; self.transform_world()
		elif k in ['up',    'S+up']:    self.shift[0] = self.shift[0] + linear_add; self.transform_world()
		elif k in ['right', 'S+right']: self.shift[1] = self.shift[1] - linear_add; self.transform_world()
		elif k in ['left',  'S+left']:  self.shift[1] = self.shift[1] + linear_add; self.transform_world()
		elif k in ['prior', 'S+prior']: self.rotate = self.rotate + linear_add; self.transform_world()
		elif k in ['next',  'S+next']:  self.rotate = self.rotate - linear_add; self.transform_world()
		elif k in ['1']: self.load_preset_cells(1)
		elif k in ['2']: self.load_preset_cells(2)
		elif k in ['z']: self.load_animal_ID(self.animal_ID)
		elif k in ['x']: self.load_cells(self.fore, is_replace=False, is_random=True)
			...
			self.fore = Board.from_data(data)
	...
	def load_animal_ID(self, ID, **kwargs):
		...
		self.load_cells(Board.from_data(self.animal_data[self.animal_ID]), **kwargs)

	def load_preset_cells(self, id, **kwargs):
		...
		self.load_cells(Board.from_values(names, params, cells), **kwargs)

	def load_cells(self, board, is_replace=True, is_random=False, zoom=None):
		self.fore = board
		if zoom == None:
			zoom = self.zoom
		...
		if board.params is not None and board.cells is not None:
			if is_replace:
				...
				self.world.clear()
			self.shift = [0, 0]
			self.rotate = np.random.random() * 360 if is_random else 0
			self.world.add(board.transform(self.rotate, self.world.params['R']), self.shift, is_random=is_random)

	def transform_world(self):
		# self.world.clear()
		if self.is_composit:
			self.world.cells = self.back.cells.copy()
			self.world.add(self.fore.transform(self.rotate, self.world.params['R']), self.shift)
		else:
			if not self.is_run:
				if self.back is None:
					...
					self.world.cells = self.back.cells.copy()
			self.world.transform(self.rotate, self.world.params['R'], shift=[-self.shift[0],-self.shift[1]], is_replace=True, reshape=False)
...
if __name__ == '__main__':
	lenia = Lenia()
	lenia.load_preset_cells(1, zoom=8)
	lenia.update_info()
	lenia.loop()
