...
class Board:
    ...
    def transform(self, tx, mode='RZSF', z_axis=Z_AXIS, is_world=False):
        ...
        if 'F' in mode and tx['flip'] != -1:
            ...
            elif tx['flip'] == 4: self.cells[extra_slice + [slice(None), slice(MIDX,None)]] = 0
...
class Automaton:
    ...
    def compile_gpu(self, A):
        ...
        try:
            ...
            # self.gpu_thr = self.gpu_api.Thread.create(interactive=True)
            # self.gpu_thr = self.gpu_api.Thread.create(device_filters={'include_devices':['AMD', 'Nvidia']})
            # platforms = self.gpu_api.get_platforms()
            # print(platforms[0].name, platforms[0].vendor, platforms[0].version, platforms[0].get_devices())
            # print(reikna.cluda.find_devices(self.gpu_api, include_devices=['AMD', 'Nvidia']))
...
class Lenia:
    ...
    def random_world_and_params(self, is_reseed=False, is_fill=False):
        ...
        for k in KERNEL:
            ...
            if self.is_search_small:
                ...
                # p['s'] = round(np.random.rand() * (0.05 - 0.01) + 0.01, 3)
    ...
    def update_window(self, show_arr=None, is_reimage=True):
        if is_reimage:
            ...
            if show_arr is not None:
                ...
                elif self.show_what==1: self.draw_world(self.automaton.potential, 0, 2*self.world.params[0]['m'], is_shift=True, is_shift_zero=True, markers=['arrow','scale','grid','colormap','params'])
                ...
                elif self.show_what>=3: self.draw_world(self.automaton.kernel[self.curr_kernel], 0, 1, markers=['scale','fixgrid','colormap','params'])
    ...
    def process_key(self, k):
        ...
        elif k in ['tab', 's+tab']: self.show_what = (self.show_what + inc_or_dec) % (3+len(KERNEL)); self.curr_kernel = max(0, self.show_what-3); self.info_type = 'params'
        ...
        elif k in ['q', 's+q']: self.world.params[self.curr_kernel]['m'] += inc_10_or_1 * 0.001; self.analyzer.new_segment(); self.check_auto_load(); self.info_type = 'params'
        elif k in ['a', 's+a']: self.world.params[self.curr_kernel]['m'] -= inc_10_or_1 * 0.001; self.analyzer.new_segment(); self.check_auto_load(); self.info_type = 'params'
        elif k in ['w', 's+w']: self.world.params[self.curr_kernel]['s'] += inc_10_or_1 * 0.0001; self.analyzer.new_segment(); self.check_auto_load(); self.info_type = 'params'
        elif k in ['s', 's+s']: self.world.params[self.curr_kernel]['s'] -= inc_10_or_1 * 0.0001; self.analyzer.new_segment(); self.check_auto_load(); self.info_type = 'params'
        ...
        elif k in ['c+f']: self.tx['R'] = self.fore.params[0]['R'] if self.fore else DEF_R; self.transform_world(); self.info_type = 'params'
        #param B [yuiop ;]
        ...
        elif k in ['semicolon']:   self.world.params[self.curr_kernel]['b'].append(0); self.adjust_b(+1); self.info_type = 'params'
        elif k in ['s+semicolon']: self.world.params[self.curr_kernel]['b'].pop();     self.adjust_b(-1); self.info_type = 'params'
        #search [c+qa]
        ...
        elif k in ['c+p']: self.automaton.is_inverted = not self.automaton.is_inverted; self.world.params[0]['T'] *= -1; self.world.params[0]['m'] = 1 - self.world.params[0]['m']; self.world.cells = 1 - self.world.cells
        ...
        elif k in ['m', 's+m']: self.is_search_small = 's+' in k; self.random_world_and_params(); self.info_type = 'params'
        elif k in ['c+m', 's+c+m']: self.is_search_small = 's+' in k; self.toggle_search(0); self.random_world_and_params(); self.info_type = 'params'
    ...
    def get_value_text(self, name):
        ...
        elif name=='colormap_id': return ["Turbo","Jet","Paul Tol's Rainbow","Vivid green/purple","Vivid red/green","Pale blue/red","Pale green/purple","Pale yellow/green","White/black","Black/white"][self.colormap_id]
        # elif name=='show_what': return ["World","Potential","Field","Kernel","World FFT","Potential FFT","Gradient","Change"][self.show_what]
            ...
            if self.markers_mode in [0,1,4,5]: st.append("Colors")
            if self.markers_mode in [0,2,4,6]: st.append("Ruler")
    ...
    def update_info_bar(self):
        ...
        if self.info_type or STATUS or self.is_show_fps:
            ...
            elif self.info_type == 'params': info_st = self.world.params2st(self.world.params[self.curr_kernel]); self.is_draw_params = True
