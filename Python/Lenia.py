...
class Lenia:
	...
	def key_press(self, event):
		...
		if s & 0x4: k = 'ctrl+' + k
		if s & 0x1: k = 'shift+' + k
		linear_add = 1 if 'shift+' in k else 10
		double_mul = 1 if 'shift+' in k else 2
		double_add = 1 if 'shift+' in k else 0
		...
		# elif k in ['f']: self.zoom = max(1, min(20, self.zoom - 1)); self.load_animal_ID(self.animal_ID)
		elif k in ['r', 'shift+r']: self.params['R'] = max(1, min(max(SIZEX,SIZEY), int(self.params['R'] * double_mul + double_add))); self.transform_world()
		elif k in ['f', 'shift+f']: self.params['R'] = max(1, min(max(SIZEX,SIZEY), int(self.params['R'] / double_mul - double_add))); self.transform_world()
		elif k in ['down',  'shift+down']:  self.shift[1] = self.shift[1] + linear_add; self.transform_world()
		elif k in ['up',    'shift+up']:    self.shift[1] = self.shift[1] - linear_add; self.transform_world()
		elif k in ['right', 'shift+right']: self.shift[0] = self.shift[0] + linear_add; self.transform_world()
		elif k in ['left',  'shift+left']:  self.shift[0] = self.shift[0] - linear_add; self.transform_world()
		elif k in ['prior', 'shift+prior']: self.rotate = self.rotate + linear_add; self.transform_world()
		elif k in ['next',  'shift+next']:  self.rotate = self.rotate - linear_add; self.transform_world()
			...
			with open('last_animal.rle', 'w') as file:
				if self.longname != '': file.write('#N '+self.longname+'\n')
			...
			self.load_animal([data['code'], data['name'], data['cname']], dict(data['params']), data['cells'])
	...
	def load_animal_ID(self, ID, is_clear=True, zoom=None):
		...
		self.load_animal([data.get('code',''), data.get('name',''), data.get('cname','')], data.get('params'), data.get('cells'), is_clear, zoom)
	...
	def load_builtin_cells(self, id, is_clear=True, zoom=None):
		...
		self.load_animal(names, params, cells, is_clear, zoom)

	def load_animal(self, names, params, cells, is_clear=True, zoom=None):
		if zoom == None:
			zoom = self.zoom
		...
		self.longname = ' | '.join(filter(None, self.names))
		if params is not None and cells is not None:
	...
	def show_world(self):
		if self.show_what == 1: self.show_panel(self.panel1, self.world, 0, 1)
		elif self.show_what == 2: self.show_panel(self.panel1, self.potential, 0, 0.3)
		elif self.show_what == 3: self.show_panel(self.panel1, self.field, -1, 1)
		elif self.show_what == 4: self.show_panel(self.panel1, self.kernel, 0, 1)
