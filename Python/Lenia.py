...
class Lenia:
	def __init__(self):
		...
		self.is_run = True
		self.is_once = False
		self.set_params(R=11, peaks=np.array([1]), mu=0.15, sigma=0.015, dt=0.1, kernel_type=0, delta_type=0)
		self.calc_kernel()
		self.clear_world()
	...
	def key_press(self, event):
		...
		elif k=='t': self.dt = self.dt * 2
		elif k=='g': self.dt = self.dt / 2
		elif k=='1': load_cells(1, multiply=4)
		elif k=='2': load_cells(2, multiply=4)
	...
	def set_params(self, R, peaks, mu, sigma, dt, kernel_type=0, delta_type=0):
		self.R = R
		self.peaks = peaks
		self.mu = mu
		self.sigma = sigma
	...
	def delta_func(self, n):
		if self.delta_type == 0:
			return np.maximum(0, 1 - (n - self.mu)**2 / (self.sigma**2 * 9) )**4 * 2 - 1
		else:
			return np.exp( - (n - self.mu)**2 / (self.sigma**2 * 2) ) * 2 - 1
	...
	def kernel_shell(self, r):
		k = len(self.peaks)
		kr = k * r
		peak = self.peaks[np.minimum(np.floor(kr).astype(int), k-1)]
		return (r<1) * self.kernel_core(kr % 1) * peak
	...
	def add_cells(self):
		w = np.shape(self.cells)[1]
		h = np.shape(self.cells)[0]
		i = MID - int(w / 2)
		j = MID - int(h / 2)
		self.world[j:j+h, i:i+w] = self.cells
	...
	def calc_once(self, is_update=True):
		...
		self.delta = self.delta_func(self.potential)
		if is_update: self.world = np.maximum(0, np.minimum(1, self.world + self.delta * self.dt))
...
lenia = Lenia()
# lenia.set_params(R=52, peaks=np.array([1/2, 2/3, 1]), mu=0.3, sigma=0.03, dt=0.1, kernel_type=0, delta_type=0)
# lenia.show_plots()
# lenia.calc_kernel()
# lenia.draw_image(lenia.kernel)
# lenia.window.mainloop()
