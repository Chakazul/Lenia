...
class Lenia:
	...
	def key_press(self, event):
		...
		elif k=='1': self.load_builtin_cells(1)
		elif k=='2': self.load_builtin_cells(2)
	...
	def load_animal(self, name, params, cells, is_clear=True, zoom=None):
		...
		if type(cells) is str:
			self.cells = self.rle_decode(cells)
		else:
			self.cells = cells
		if is_clear:
			self.clear_world()
	...
	def start_record(self):
		...
		self.record_seq = 1
		video_path = os.path.join(RECORD_ROOT, '{}.mp4'.format(self.record_ID))
		codec = cv2.VideoWriter_fourcc(*'avc1')
		self.video = cv2.VideoWriter(video_path, codec, 24, (SIZEX,SIZEY))  # .avi *'DIVX' / .mp4 *'mp4v' *'avc1'
