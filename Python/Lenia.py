...
class Lenia:
	def __init__(self):
		...
		self.read_animals()
		self.params = {'R':11, 'b':[1], 'm':0.15, 's':0.015, 'dt':0.1, 'kn':0, 'gn':0}
		self.calc_kernel()
		self.clear_world()
		self.create_figure()
	...
	def key_press(self, event):
		...
		if k=='escape': self.close()
		...
		elif k=='q': self.params['m'] += 0.01
		elif k=='a': self.params['m'] -= 0.01
		elif k=='w': self.params['s'] += 0.001
		elif k=='s': self.params['s'] -= 0.001
		elif k=='shift+q': self.params['m'] += 0.001
		elif k=='shift+a': self.params['m'] -= 0.001
		elif k=='shift+w': self.params['s'] += 0.0001
		elif k=='shift+s': self.params['s'] -= 0.0001
		elif k=='t': self.params['dt'] = self.params['dt'] * 2
		elif k=='g': self.params['dt'] = self.params['dt'] / 2
		elif k=='r': self.zoom = np.clip(self.zoom + 1, 1, 20); self.load_animal_ID(self.animal_ID)
		elif k=='f': self.zoom = np.clip(self.zoom - 1, 1, 20); self.load_animal_ID(self.animal_ID)
		elif k=='1': self.load_cells(1)
		elif k=='2': self.load_cells(2)
		elif k=='z': self.load_animal_ID(self.animal_ID)
		elif k=='c': self.load_animal_ID(self.animal_ID - 1)
		elif k=='v': self.load_animal_ID(self.animal_ID + 1)
		elif k in ['ctrl+c', 'cmd+c', 'shift+ctrl+c', 'shift+cmd+c']:
			A = self.crop_cells(self.world, 1/255)
			rle_st = self.rle_encode(A, 'shift+' not in k)
			params_st = ','.join(['%s=%s' % (k,str(v)) for k,v in self.params.items()])
			...
			self.load_animal(json.loads(self.clip_st))
		elif '_l' in k or '_r' in k or k in ['app']: pass
	...
	def load_animal_ID(self, ID):
		self.animal_ID = np.clip(ID, 0, len(self.animals)-1)
		animal = self.animals[self.animal_ID]
		if 'cells' in animal:
			self.load_animal(animal)
		else:
			self.win.title('Lenia')

	def load_animal(self, data):
		self.name = data['name']
		self.params = dict(data['params'])
		if type(self.params['b']) is str:
			self.params['b'] = [float(frac.Fraction(st)) for st in self.params['b'].split(',')]
		self.cells = self.rle_decode(data['cells'])
		self.clear_world()
		if self.zoom > 1:
			self.double_cells(self.zoom)
		self.calc_kernel()
		self.add_cells(self.cells)
		self.win.title('Lenia  ['+self.name+'] x'+str(self.zoom))

	def rle_encode(self, A, is_shorten=True):
	...
	def calc_once(self):
		...
		self.world = np.clip(self.world + self.field * self.params['dt'], 0, 1)
