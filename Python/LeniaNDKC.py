...
class Board:
    ...
    def transform(self, tx, mode='RZSF', z_axis=Z_AXIS, is_world=False):
        ...
        if 'Z' in mode and tx['R'] != self.params[0]['R']:
            ...
            for c in CHANNEL:
                self.cells[c] = scipy.ndimage.zoom(self.cells[c], tx['R'] / self.params[0]['R'], order=0)
            if is_world:
    ...
    def calc_once(self, is_update=True):
        ...
        for c in CHANNEL:
            ...
            if self.is_soft_clip:
                A_new[c] = Automaton.soft_clip(A_new[c], 0, 1, 1/dt)  # A_new = A + dt * Automaton.soft_clip(D, -A/dt, (1-A)/dt, 1)
            else:
                A_new[c] = np.clip(A_new[c], 0, 1)  # A_new = A + dt * np.clip(D, -A/dt, (1-A)/dt)
            if self.world.param_P > 0:
                A_new[c] = np.around(A_new[c] * self.world.param_P) / self.world.param_P
...
class Analyzer:
    ...
    def calc_stats(self, polar_what=0, psd_x='m', psd_y='g', is_welch=True):
        ...
        self.is_empty = any(A0.sum() < 1e-4 for A0 in self.world.cells)  #EPSILON
        border_sum = 0
        for d in range(DIM):
...
class Recorder:
    ...
    def start_record(self):
        ...
        self.record_id = '{name}-{time}'.format(name=self.world.names[0].split('(')[0], time=datetime.datetime.now().strftime('%Y%m%d-%H%M%S'))
        self.record_seq = 1
    ...
    def save_image(self, img, filename=None):
        self.record_id = '{name}-{time}'.format(name=self.world.names[0].split('(')[0], time=datetime.datetime.now().strftime('%Y%m%d-%H%M%S-%f'))
...
class Lenia:
    ...
    def __init__(self):
        ...
        self.search_algo = 0  # 0=global, 1=depth, 2=breadth, 3=depth+breadth 4=GA(max), 5=GA(avg), 6=GA(std)
    ...
    def load_found_animal_id(self, world, id, **kwargs):
        ...
        if self.world.names[0] == '':
            self.world.names[0] = ['Found #' + str(self.found_animal_id + 1)]
    ...
    def random_world(self, is_reseed=False, is_fill=False):
        ...
        if is_reseed:
            if self.world.names[2].startswith('seed:'):
                self.last_seed = self.world.names[2].split(':')[-1]
        else:
            self.last_seed = self.random_hex()
        try:
        ...
        if is_fill:
            ...
            for i in range(np.random.randint(15, 40)):
                ...
                # shift = [np.random.randint(border, size - border) - size//2 if border < size - border else 0  for size in reversed(SIZE)]
                self.world.add(Board.from_values(rand), shift=shift)

        self.world_updated()
        self.world.names[1] = ''
        self.world.names[2] = 'seed:'+str(self.last_seed)
    ...
    def random_params(self, is_incremental=False):
        ...
        if is_incremental:  # local
            ...
            for k in KERNEL:
            #for k in [np.random.randint(KN)]:
                p = self.world.params[k]
                c0, c1 = p.get('c', [0, 0])
                #if c0 != c1:
                rand = np.random.randint(B_change) if B_change>0 else -1
                if rand == 0 and len(p['b']) > 1:
                    p['b'].pop()
                    if len(p['b']) == 1:
                        p['b'] = [1]
                elif rand == 1 and len(p['b']) < 3:
                    p['b'].append(0)
                if p['b'] != [1]:
                    ...
                    if max(p['b']) != 1:
                        for bi in reversed(range(len(p['b']))):
                            if p['b'][bi] == Fraction(11, 12):
                                p['b'][bi] = 1
                                break
                self.random_local(p, 'm', delta=0.02  if is_small else 0.05,  vmin=0.1,  vmax=1.0,  digit=3)
                self.random_local(p, 's', delta=0.002 if is_small else 0.005, vmin=0.01, vmax=1.00, digit=4)
                self.random_local(p, 'h', delta=0.1, vmin=0.1, vmax=1.0, digit=2)
                self.random_local(p, 'r', delta=0.1, vmin=0.5, vmax=1.0, digit=2)
        else:  # global
            for k in KERNEL:
                p = self.world.params[k]
                c0, c1 = p['c']
                ...
                self.random_global(p, 'r', vmin=0.5, vmax=1.0, digit=2)
                # p['r'] = 1
        self.automaton.calc_kernel()
        self.world_updated()
        self.update_lineage()
    ...
    def mutate_world_from_leaderboard(self):
        # print(["{:.3f} {}".format(e['val'], e['world'].names[0] if e['world'] is not None else 'X') for e in self.leaderboard])
            ...
            for i in order:
                ...
                if world is not None:
                    if w1 is None: w1 = world
                        ...
                        self.world.params = []
                        self.world.names = [self.clean_code(w1.names[0])+"x"+self.clean_code(w2.names[0]), '', '']
                        self.update_lineage()
                        for c in CHANNEL:
    ...
    def do_search(self):
        ...
        if self.search_mode == +1:
            ...
            if self.analyzer.is_empty or self.analyzer.is_full:
            ...
            elif self.search_algo in [4, 5, 6] and self.automaton.gen >= 250 and self.search_stage == 1:
                self.is_show_search = True
                self.search_stage = 2
                self.automaton.reset()
                self.analyzer.reset()
            elif self.search_algo in [4, 5, 6] and self.automaton.gen >= 500:
                self.is_show_search = True
                self.search_stage = 1
                self.search_total += 1
                # self.update_lineage()
                ...
                self.mutate_world_from_leaderboard()
            elif self.automaton.gen == 10 or self.automaton.gen % 50 == 0:
                self.is_show_search = True
    ...
    def update_lineage(self, name='', cname=''):
        ...
        prev_code = self.clean_code(self.world.names[0])
        if self.search_algo in [0]:
            self.world.names = [new_code, '', 'seed:'+self.last_seed]
        elif self.search_algo in [1, 2, 3]:
            self.world.names = [new_code+'<'+prev_code, '', '']
        elif self.search_algo in [4, 5, 6]:
            self.world.names = [new_code+'<'+prev_code, name, cname]
    ...
    def is_show_rgb(self):
        return self.show_what==0 and CN>1
    ...
    def show_which_channels(self, A):
        ...
        if self.is_show_rgb():
            ...
            for c in CHANNEL:
                m = self.channelmaps[(c + self.channel_shift) % 3 + group * 3]
                # r = np.maximum(r, A[c] * m[0])
                r += A[c] * m[0]
                g += A[c] * m[1]
                b += A[c] * m[2]
            r += self.channelbg[group][0] / (DIM-1)
            g += self.channelbg[group][1] / (DIM-1)
            b += self.channelbg[group][2] / (DIM-1)
            A = [r, g, b]
        return A
    ...
    def update_window(self, show_arr=None, is_reimage=True):
        if is_reimage:
            ...
            #     grad = np.sqrt(xgrad**2 + ygrad**2) * self.world.params[0]['R'] * 1.5
            ...
            if show_arr is not None:
                ...
                elif self.show_what==1: self.draw_world(self.automaton.potential[self.show_kernel], 0, 2*self.world.params[self.show_kernel]['m'], is_shift=True, is_higher_zero=True, markers=['arrow','scale','grid','colormap','params'])
                elif self.show_what==2: self.draw_world(self.automaton.field[self.show_kernel], field_lo, 1, is_shift=True, markers=['arrow','scale','grid','colormap','params'])
                elif self.show_what==3: self.draw_world(self.automaton.kernel[self.show_kernel], 0, 1, markers=['scale','fixgrid','colormap','params'])
    ...
    def copy_world(self, type='JSON'):
        ...
        if type == 'JSON':
            ...
            for (i, ch) in enumerate(A.cells):
                ...
                self._recur_write_csv(0, writer, ch)
    ...
    def process_key(self, k):
        global STATUS
        is_life = (self.world.params[0].get('kn') == 4)
        ...
        elif k in ['s+period'] and self.is_show_rgb(): self.channel_shift = (self.channel_shift + 1) % len(self.channelmaps); self.info_type = 'channel'
        elif k in ['s+comma']  and self.is_show_rgb(): self.channel_shift = (self.channel_shift - 1) % len(self.channelmaps); self.info_type = 'channel'
        elif k in ['tab', 's+tab']: self.show_what = (self.show_what + inc_or_dec) % 4
        elif k in ['quoteleft', 's+quoteleft']: self.show_kernel = (self.show_kernel + inc_or_dec) % len(KERNEL); self.info_type = 'params'
        elif k in ['s+c+tab']: self.show_what = 0; self.show_kernel = 0; self.colormap_id = 0; self.channel_shift = 0; self.info_type = 'channel'
        ...
        elif k in ['s+c+d']: self.world.param_P = 0; self.info_type = 'info'
        elif k in ['s+c+r']: self.tx['R'] = DEF_R; self.transform_world(); self.info_type = 'params'
        elif k in ['s+c+f']: self.tx['R'] = self.fore.params[0]['R'] if self.fore else DEF_R; self.transform_world(); self.info_type = 'params'
        elif k in ['s+c+t']: self.world.params[0]['T'] = self.world.params[0]['T'] / 2; self.analyzer.new_segment(); self.info_type = 'params'
        #param B [yuiop ;]
        ...
        elif k in ['semicolon']:   self.world.params[self.show_kernel]['b'].append(0); self.adjust_b(+1); self.info_type = 'params'
        elif k in ['s+semicolon']: self.world.params[self.show_kernel]['b'].pop();     self.adjust_b(-1); self.info_type = 'params'
        #search [c+qa]
        ...
        #options [c+yuiop]
        elif k in ['c+y', 's+c+y']: self.world.params[0]['kn'] = (self.world.params[0]['kn'] + inc_or_dec - 1) % len(self.automaton.kernel_core) + 1; self.automaton.calc_kernel(); self.info_type = 'kn'
        elif k in ['c+u', 's+c+u']: self.world.params[0]['gn'] = (self.world.params[0]['gn'] + inc_or_dec - 1) % len(self.automaton.growth_func) + 1; self.info_type = 'gn'
        ...
        elif k in ['minus']: self.tx['flip'] = 4 if self.tx['flip'] != 0 else -1; self.transform_world()
        elif k in ['s+minus']: self.tx['flip'] = 5 if self.tx['flip'] != 0 else -1; self.transform_world()
        #autocenter [']
        ...
        elif k in ['s+z']: self.load_animal_id(self.world, self.animal_id); self.world.params[0]['kn'] = self.world.params[0]['gn'] = 2; self.automaton.calc_kernel(); self.info_type = 'animal'
        elif k in ['b']: self.load_found_animal_id(self.world, self.found_animal_id + 1); self.world_updated(); self.info_type = 'animal'
        elif k in ['s+b']: self.load_found_animal_id(self.world, self.found_animal_id - 1); self.world_updated(); self.info_type = 'animal'
        elif k in ['c+b']: self.found_animal_id = len(self.found_animal_data) if self.found_animal_id==0 else 0; self.load_found_animal_id(self.world, self.found_animal_id); self.world_updated(); self.info_type = 'animal'
        elif k in ['c+backspace', 'c+delete']: self.delete_found_animal(code=self.world.names[0]); self.load_found_animal_id(self.world, self.found_animal_id); self.world_updated(); self.info_type = 'animal'
    ...
    def create_menu(self):
        ...
        ''' menu item: variable|text|key(actual)|key(displayed)
    ...
    def get_info_st(self):
        ...
        if P == '0': P = '∞'
        return "gen={}, t={}s, dt={}s, sampl={}, P={}, world={}, pixel={}".format(self.automaton.gen, self.automaton.time, 1/self.world.params[0]['T'], self.samp_freq, P, "x".join(str(size) for size in SIZE), PIXEL)

    def get_time_st(self):
        return "gen={}, t={}s, dt={}s, sampl={}".format(self.automaton.gen, self.automaton.time, 1/self.world.params[0]['T'], self.samp_freq)
    ...
    def update_info_bar(self):
        ...
        if self.info_type or STATUS or self.is_show_fps:
            ...
            elif self.info_type == 'params': info_st = self.world.params2st(self.world.params[self.show_kernel]); self.is_draw_params = True
